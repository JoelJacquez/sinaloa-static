...
function inter_slide(){
	...
	//erick: movi delay para retardar cadena original
	...
	if (this.dom){ //if modern browsers (ie: Firefox, IE5+)
		...
		document.write('<div style="width:auto !Important; height:auto !Important; overflow:hidden;" align="center"><div id="master'+this.issid+'" style="position:relative;'+dims+'overflow:hidden;margin:0 auto;"><div id="'+this.canvasbase+'_0" style="position:absolute;'+dims+'top:0;left:0;'+ief+'background-color:'+(imgs.fadecolor? imgs.fadecolor : 'white')+'"><\/div><div id="'+this.canvasbase+'_1" style="position:absolute;'+dims+'top:0;left:0;'+ief+'background-color:'+(imgs.fadecolor? imgs.fadecolor : 'white')+'"><\/div><\/div><\/div>');
		if(this.descriptions)
		document.write(brs+'<div style=" width:auto !Important; height:auto !Important; overflow:hidden;" align="center">'+this.dp+'<span id="imgdsc'+this.issid+'">&nbsp;<\/span></div>\n');
...
inter_slide.prototype.controls=function(){
	...
	document.write('<div align="center"><input class="btn_slideshow_anterior"  id="prev'+this.issid+'" disabled type="button" value="    Anterior" onclick="iss['+this.issid+'].changeimg(false, \'nav\');">\n'+
	(this.no_auto? '&nbsp;&nbsp;&nbsp;' : ' <input class="btn_slidesshow_stopandplay" id="gostp'+this.issid+'" type="button" value="Stop" onclick="iss['+this.issid+'].gostop(this);">\n')+
	' <input class="btn_slideshow_siguiente" id="next'+this.issid+'" disabled type="button" value="Siguiente     " onclick="iss['+this.issid+'].changeimg(true, \'nav\');">'+(this.jumpto? brs+'Jump to: <input class="btn_slidesshow_play" disabled type="text" size="'+this.imgs.length.toString().length+'" id="goto'+this.issid+'" onkeydown="iss['+this.issid+'].jumper(event);"><input id="go'+this.issid+'" disabled type="button" onclick="iss['+this.issid+'].jumper();" value="GO">' : '')+'<\/div>');
...
inter_slide.prototype.imgload=function(_el){
	if(!_el&&!this.imgel)
	return;
	var  el=_el? _el : this.imgel;
